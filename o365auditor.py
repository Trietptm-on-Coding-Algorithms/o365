...
# There are likely a lot more than this.
appID = {
...
externalAccess = {
    False: "Cmdlet run by someone in your organization.",
    True: "Cmdlet run by DC personnel, DC service account, or delegated admin."
}
...
recordType = {
    ...
    27: "MicrosoftTeamsSettingsOperation"
}

def FileOperations(jd, data):
    '''
    This covers the following Operations as the have the same fields.
    ...
    '''
    ctime = jd['CreationTime']
    operation = jd['Operation']
    clientip = jd['ClientIP']
    userid = jd['UserId']
    usertype = userType[jd['UserType']]
    srcfile = jd['SourceFileName']
    rectype = recordType[jd['RecordType']]
    # workload = jd['Workload']
    # objectid = jd['ObjectId']
    evtsource = jd['EventSource']
    # webid = jd['WebId']
    # useragent = jd['UserAgent']
    relativeurl = '/' + jd['SourceRelativeUrl'] + '/'

    return (ctime, operation, evtsource, rectype, clientip, userid, usertype, relativeurl, srcfile, 'None')

def FileSyncDownloadedFull(jd, data):
    ctime = jd['CreationTime']
    operation = jd['Operation']
    clientip = jd['ClientIP']
    userid = jd['UserId']
    usertype = userType[jd['UserType']]
    syncbytes = jd['FileSyncBytesCommitted']
    srcfile = jd['SourceFileName']
    rectype = recordType[jd['RecordType']]
    # workload = jd['Workload']
    # objectid = jd['ObjectId']
    ...
    return(ctime, operation, evtsource, rectype, clientip, userid, usertype, relativeurl, srcfile, syncbytes)

def parseExtended(extendprop, data):
    extList = []

    if  extendprop[1]['Name'] == 'FlowTokenScenario':
        authmethod = extendprop[2]['Value']
        requesttype = extendprop[3]['Value']
        resultdetail = extendprop[4]['Value']
        tokenscenario = extendprop[1]['Value']

        extList.append([authmethod,requesttype,resultdetail,tokenscenario])

    elif extendprop[1]['Name'] == 'UserAuthenticationMethod':
        authmethod = extendprop[1]['Value']
        requesttype = extendprop[2]['Value']
        resultdetail = extendprop[3]['Value']
        try:
            keepmesignedin = extendprop[4]['Value']
        except IndexError:
            keepmesignedin = 'None'

        extList.append([authmethod,requesttype,resultdetail,keepmesignedin])

    return(extList)

def UserLoggedIn(jd, data):
    ctime = jd['CreationTime']
    operation = jd['Operation']
    clientip = jd['ClientIP']
    userid = jd['UserId']
    usertype = userType[jd['UserType']]
    rectype = recordType[jd['RecordType']]
    extend = parseExtended(jd['ExtendedProperties'], data)
    # actor = jd['Actor']
    appid = jd['ApplicationId']
    ...
    return(ctime, operation, appid, rectype, clientip, userid, usertype, 'None', extend[0][1], 'None')

def PageViewed(jd, data):
    ctime = jd['CreationTime']
    operation = jd['Operation']
    clientip = jd['ClientIP']
    userid = jd['UserId']
    ...
    return(ctime, operation, evtsource, rectype, clientip, userid, usertype, 'None', objectid, 'None')

def NewInboxRule(jd, data):
    ctime = jd['CreationTime']
    operation = jd['Operation']
    clientip = jd['ClientIP']
    userid = jd['UserId']
    usertype = userType[jd['UserType']]
    rectype = recordType[jd['RecordType']]
    originatingserver = jd['OriginatingServer']
    # objectid = jd['ObjectId']
    # externalaccess = externalAccess[jd['ExternalAccess']]
    rulename = jd['Parameters'][2]['Value']
    words = jd['Parameters'][3]['Value']
    # deletemessage = jd['Parameters'][4]['Value']
    # markread = jd['Parameters'][5]['Value']
    # stopprocessingrules = jd['Parameters'][6]['Value']
    evtsource = 'None'

    return(ctime, operation, evtsource, rectype, clientip, userid, usertype, originatingserver, \
           rulename, words)

def VideoRequested(jd, data):
    ctime = jd['CreationTime']
    operation = jd['Operation']
    clientip = jd['ClientIP']
    userid = jd['UserId']
...
def processOutput(masterResults, workbook, outfile):

    wb = workbook
    wb.create_sheet('O365 Logs')
    ws = wb['O365 Logs']

    # Column header labels.
    ws.append(['c_time', 'operation', 'evt_src', 'rec_type', 'client_ip', 'user_id', 'user_type', \
              'relative_url', 'src_file', 'misc'])

    for results in masterResults:
        ws.append(results)
...
mappingDict = {
    'FileModifiedExtended': FileOperations,
    'FileAccessedExtended': FileOperations,
    'FileModified': FileOperations,
    'FileAccessed': FileOperations,
    'FileSyncDownloadedFull': FileSyncDownloadedFull,
    'FileUploaded': FileOperations,
    'FileSyncUploadedFull': FileOperations,
    'UserLoggedIn': UserLoggedIn,
    'PageViewed': PageViewed,
    'New-InboxRule': NewInboxRule,
    "VideoRequested": VideoRequested,
    "FilePreviewed": FileOperations,
    "FileDeleted": FileOperations,
    "FileSyncDownloadedPartial": FileOperations
}

def main():
    parser = argparse.ArgumentParser(description='Parse Office 365 csv exported audit logs.')
    parser.add_argument('-f', '--file', required=True, \
                        help='Path to the exported O365 CSV audit log file.')
    parser.add_argument('-o', '--out', required=True, \
                        help='Name of the out file you want to save everything to xlsx.')
    args = parser.parse_args()

    infile = args.file

    if not args.out.endswith('.xlsx'):
        outfile = args.out+'.xlsx'
    elif args.out.endswith('.xlsx'):
        outfile = args.out

    workbook = processCSV(infile)

    sheet = workbook.active

    masterResults = []

    for entries in sheet['D']:
        if (entries.value).startswith('{"'):
            jd = json.loads(entries.value)
